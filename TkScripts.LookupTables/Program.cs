...
internal static class App
{
    ...
    public static async Task Run(string output = "output", CancellationToken cancellationToken = default)
    {
        ...
        tasks[0] = store.WriteResults(new GameDataDeltaGenerator(), gamePaths);
        tasks[1] = store.WriteResults(new GameDataDeltaGenerator(), gamePaths, compress: true);
        // tasks[3] = store.WriteResults(new ChecksumGenerator(), gamePaths);
        // tasks[4] = store.WriteResults(new PackFileLookupGenerator(), gamePaths, compress: true);
        // tasks[5] = store.WriteResults(new MaterialDiffGenerator(), gamePaths);
